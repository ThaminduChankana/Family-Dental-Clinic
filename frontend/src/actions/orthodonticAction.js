...
export const listOrthodontics = () => async (dispatch, getState) => {
	try {
		...
		// Remove the config
		const { data } = await axios.get(`http://localhost:5000/user/doctor/treatment/orthodontic/get`);
...
export const createOrthodonticAction =
	(nic, firstVisit, fullCost, paid, facialExamination, followUpVisits, remark) => async (dispatch, getState) => {
		try {
			...
			// Remove the config
			const { data } = await axios.post(`http://localhost:5000/user/doctor/treatment/orthodontic/create`, {
				nic,
				firstVisit,
				fullCost,
				paid,
...
export const updateOrthodonticAction =
	(id, nic, fullCost, paid, firstVisit, facialExamination, followUpVisits, remark) => async (dispatch, getState) => {
		try {
			...
			const { data } = await axios.put(`http://localhost:5000/user/doctor/treatment/orthodontic/get/${id}`, {
				nic,
				fullCost,
				paid,
				firstVisit,
...
export const deleteOrthodonticAction = (id) => async (dispatch, getState) => {
	try {
		...
		const { data } = await axios.delete(`http://localhost:5000/user/doctor/treatment/orthodontic/get/${id}`);
